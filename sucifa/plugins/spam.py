...
async def bigspam(hell):
    if not hell.text[0].isalpha() and hell.text[0] not in ("/", "#", "@", "!"):
        hell_msg = hell.text
        sucifa_count = int(hell_msg[9:13])
        hell_spam = str(hell.text[13:])
        for i in range(1, sucifa_count):
...
CmdHelp("spam").add_command(
  ...
  "bigspam", "<count> <text>", "Sends the text 'X' number of times. This what Andini is known for. The Best BigSpam Ever", ".bigspam 5000 Hello"
).add_info(
  "Spammers Commands"
).add_warning(
