...
async def send(event):
    ...
    omk = f"**• Plugin name ≈** `{input_str}`\n**• Uploaded by ≈** {hell_mention}\n\n⚡ **[ SFA-Enterprise ]({chnl_link})** ⚡"
    the_plugin_file = "./sucifa/plugins/{}.py".format(input_str)
    ...
    if event.reply_to_msg_id:
        try:
            downloaded_file_name = await event.client.download_media(  # pylint:disable=E0602
                ...
                "./sucifa/plugins/"  # pylint:disable=E0602
            )
...
async def uninstall(kraken):
    ...
    dir_path =f"./sucifa/plugins/{shortname}.py"
    try:
...
CmdHelp("core").add_command(
  "install", "<reply to a .py file>", "Installs the replied python file if suitable to Andini <3 codes."
).add_command(
  "uninstall", "<plugin name>", "Uninstalls the given plugin from Andini <3. To get that again do .restart", "uninstall alive"
).add_command(
  ...
  "cmds", None, "Gives out the list of modules in Andini <3."
).add_warning(
