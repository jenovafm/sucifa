...
import asyncio
from sucifa.sql.gban_sql import is_gbanned, gbaner, ungbaner, all_gbanned
from sucifa.sql import gmute_sql as gsql
from . import *
    ...
    async for gfuck in event.client.iter_dialogs():
        if gfuck.is_group or gfuck.is_channel:
            try:
                ...
                pass
    gbaner(userid)
    gmsg = f"🥴 [{name}](tg://user?id={userid}) *[Error] User is toxic** {hell_mention} **silahkan pergi... 😏**\n\n📍 Added to Gban Watch!!\n**🔰 Total Chats :**  `{chats}`"
    if reason != "":
        gmsg += f"\n**🔰 Reason :**  `{reason}`"
