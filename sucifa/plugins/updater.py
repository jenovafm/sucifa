...
from sucifa.helpers import runner
from . import *
...
async def update(event, repo, ups_rem, ac_br):
    ...
    await event.edit(
        "✅ Successfully updated Andini <3!\n\nBot is restarting please wait for a minute."
    )
    args = [sys.executable, "-m", "sucifa"]
...
async def upstream(event):
    ...
    if HEROKU_API_KEY is None or HEROKU_APP_NAME is None:
        return await edit_or_reply(
            event, "Set  `HEROKU_APP_NAME`  and  `HEROKU_API_KEY`  to update Andini 🥴"
        )
    try:
    ...
    if conf == "" and not force_update:
        ...
        await event.delete()
        return await event.respond(f"🌚 Do `{hl}update build` to update your **Andini <3** !!")

    if force_update:
        await event.edit(
            "`Force-Updating Andini <3. Please wait...`"
        )
    if conf == "now":
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit(f"**Your Andini <3 Is UpToDate**\n\n**Version :**  __{hell_ver}__\n**Oɯɳҽɾ :**  {hell_mention}")
    else:
...
async def upstream(event):
    ...
    off_repo = "https://github.com/jenovafm/suciandini"
    os.chdir("/app")
    ...
    ups_rem = repo.remote("upstream")
    ups_rem.fetch(ac_br)
    await event.edit(f"**Andini <3 Docker Build In Progress... Type** `{hl}ping`  **after 5 mins to check if Bot is working!**")
...
CmdHelp("update").add_command(
  ...
  "update now", None, "Soft-Update Your Andini <3. Basically if you restart dyno it will go back to previous deploy."
).add_command(
  "update build", None, "Hard-Update Your Andini <3. This won't take you back to your previous deploy. This will be triggered even if there is no changelog."
).add_info(
  "Andini <3 Updater."
).add_warning(
  "✅ Harmless Module."
