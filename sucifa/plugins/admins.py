...
from . import *
from sucifa.sql.mute_sql import is_muted, mute, unmute
...
async def muth(hell):
    if hell.is_private:
        ...
        if hell.chat_id == bytewritter:
            return await eod(hell, "Getting drunk ? 🥴")
        try:
            mute(hell.chat_id, hell.chat_id)
        except Exception as e:
            await eor(hell, f"**Error **\n`{str(e)}`")
        else:
            await eor(hell, "**[ Spammer ? ] 🥴\n`**｀-´)⊃━☆ﾟ.*･｡ﾟ **`")
    else:
