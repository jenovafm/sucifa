...
async def download(event):
    ...
    if Config.GIT_REPO_NAME is None:
        ...
        return
    sucifa = await eor(event, "Processing ...")
    ...
    try:
        ...
        await sucifa.edit("Committing to Github....")
        await git_commit(downloaded_file_name, sucifa)


async def git_commit(file_name, sucifa):
    content_list = []
    ...
    for i in content_list:
        ...
        if i == 'ContentFile(path="' + file_name + '")':
            return await sucifa.edit("`File Already Exists`")
            create_file = False
    file_name = "sucifa/plugins/" + file_name
    if create_file == True:
        ...
        try:
            ...
            await eod(sucifa, "Cannot Upload Plugin")
    else:
        return await eod(sucifa, "`Committed Suicide`")
