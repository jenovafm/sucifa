...
hell_pic = Config.PMPERMIT_PIC or "https://i.ibb.co/8m4Wssg/uci.jpg"
...
if Config.BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query == "sucifa_help":
            rev_text = query[::-1]
            ...
            result = builder.article(
                ...
                text=f"**⚡ Andini <3 ⚡**",
                buttons=[
                    [Button.url("📑 Queen 📑", "https://instagram.com/suciandni_")],
                    [Button.url("🚀 King 🚀", "https://instagram.com/jenovafm")],
                ],
            )
    ...
    async def on_pm_click(event):
        if event.query.user_id == bot.uid:
            ...
            await event.edit(
                f"🔰 This is Andini PM Security for {hell_mention} to keep away unwanted retards from spamming PM..."
            )
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid or event.query.user_id in Config.SUDO_USERS:
            ...
            await event.edit(f"**⚜️ Andini <3 Service Closed ⚜️**\n\n**Bot Of :**  {hell_mention}\n\n        [©️SFA-Enterprise️]({chnl_link})", buttons=veriler, link_preview=False)
        else:
            reply_pop_up_alert = "Suci Fitri Andini <3"
            await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
